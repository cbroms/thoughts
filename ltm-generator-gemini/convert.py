...
index_links = "\n".join(list(map(lambda x: "=> /thought/{}.gmi".format(x), index)))
...
for name in glob.glob("../wm/*.md"):
    ...
    updates = post.metadata["updates"]
    places = post.metadata["places"]

    backlinks = "\n".join(
        list(map(lambda x: "=> /thought/{}.gmi".format(x), post.metadata["backlinks"]))
    )

    if updates != None and updates[-1] != None:
        parsed = dateutil.parser.isoparse(updates[-1])
